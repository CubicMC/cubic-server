...
{
    return std::find(INVOKER_REQUIRED_SELECTOR.begin(), INVOKER_REQUIRED_SELECTOR.end(), str)
        != INVOKER_REQUIRED_SELECTOR.end();
}
...
void fillSingleSelector(
    const std::string &selector, Entity &out, const std::vector<std::shared_ptr<Entity>> &entities,
