...
void Time::autocomplete(UNUSED std::vector<std::string> &args, std::shared_ptr<Player> invoker) const
{
    if (invoker)
        return;
    else
...
int setTimeFromArg(std::string value, int multiplier, std::shared_ptr<Player> invoker)
{
    int time = 0;
    try {
...
int setTimeToAdd(std::string timeToAdd, std::shared_ptr<Player> invoker)
{
    int time = 0;
...
int setTimeToSet(const std::string &timeToSet, std::shared_ptr<Player> invoker)
{
    int time = 0;
...
void checkArgsTime(std::vector<std::string> &args, std::shared_ptr<Player> invoker)
{
    int time = 0;
    if (args[0] == "add") {
...
void Time::execute(std::vector<std::string> &args, std::shared_ptr<Player> invoker) const
{
    if (args.size() != 2) {
        return;
    }
...
void Time::help(UNUSED std::vector<std::string> &args, std::shared_ptr<Player> invoker) const
{
    if (invoker) {
