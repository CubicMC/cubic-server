...
void command_parser::Help::autocomplete(UNUSED std::vector<std::string> &args, std::shared_ptr<Player> invoker) const
{
    if (invoker)
        return;
    else
...
void command_parser::Help::execute(std::vector<std::string> &args, std::shared_ptr<Player> invoker) const
{
    if (args.empty()) {
        if (invoker) {
...
void command_parser::Help::help(UNUSED std::vector<std::string> &args, std::shared_ptr<Player> invoker) const
{
    if (invoker) {
        if (invoker->isOperator())
