...
void Deop::autocomplete(UNUSED std::vector<std::string> &args, UNUSED std::shared_ptr<Player> invoker) const { }

void Deop::execute(std::vector<std::string> &args, std::shared_ptr<Player> invoker) const
{
...
void Deop::help(UNUSED std::vector<std::string> &args, std::shared_ptr<Player> invoker) const
{
    if (invoker) {
