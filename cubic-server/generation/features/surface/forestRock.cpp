        ...
        for (int x = 0; x < SECTION_WIDTH; x++) {
            for (int y = CHUNK_HEIGHT_MAX - 5; CHUNK_HEIGHT_MIN <= y; y--) {
                ...
                if (block == Blocks::Air::toProtocol()
                    || block == Blocks::SpruceLog::toProtocol(Blocks::SpruceLog::Properties::Axis::Y))
                    continue;
                else {
...
void ForestRock::generateRock(std::vector<world_storage::ChunkColumn *>)
{
    ...
    for (const auto &block : rock) {
        if (rockEmplacement.x + block.pos.x < 0 || rockEmplacement.x + block.pos.x >= world_storage::SECTION_WIDTH
            || rockEmplacement.z + block.pos.z < 0 || rockEmplacement.z + block.pos.z >= world_storage::SECTION_WIDTH)
            continue;
        // the following if may change/disappear
        if (_chunk.getBlock({ rockEmplacement.x + block.pos.x, rockEmplacement.y + block.pos.y,
                              rockEmplacement.z + block.pos.z })
            == Blocks::MossyCobblestone::toProtocol())
            continue;
        _chunk.modifyBlock(
            { rockEmplacement.x + block.pos.x, rockEmplacement.y + block.pos.y, rockEmplacement.z + block.pos.z },
            block.block
        );
    }
    ...
    for (int x = -1; x <= 1; x++) {
        for (int z = -1; z <= 1; z++) {
            ...
            if ((!bigRock && abs(x) == 1 && abs(z) == 1))
                continue;
            rock.emplace_back(generation::Generator::FeatureBlock{
                {x, y, z},
                mossy
            });
        }
        ...
        for (int countX = x; countX <= 1; countX++) {
            for (int countZ = z; countZ <= 1; countZ++) {
                if ((pos.x % 2 == 0 && countZ == _generator.getRandomizer() % 2 && abs(countY) == pos.y % 2)
                    || (_generator.getRandomizer() % 2 != abs(countZ) && abs(countY) != pos.y % 2
                        && abs(countX) % 2 == _generator.getRandomizer() % 2)
...
void ForestRock::buildDiversifiedRocks(
