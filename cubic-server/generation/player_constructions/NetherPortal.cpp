...
Frame NetherPortal::getFrame(Position pos)
{
    ...
    if (nextBlockX == Blocks::Obsidian::toProtocol() && topNextBlockX == Blocks::Air::toProtocol()
        && nextBlockZ != Blocks::Obsidian::toProtocol()) {
        LDEBUG("Axis X");
    ...
    if (nextBlockZ == Blocks::Obsidian::toProtocol() && topNextBlockZ == Blocks::Air::toProtocol()
        && nextBlockX != Blocks::Obsidian::toProtocol()) {
        LDEBUG("Axis Z");
            ...
            if (frame.direction == POSITIVE_POS && frame.axis == AXIS_X) {
                ...
                if (block == Blocks::Air::toProtocol()
                    ...
                    blocksArray.push_back({
                        {pos.x - 1 + x, pos.y + y, pos.z},
                        Blocks::NetherPortal::toProtocol(Blocks::NetherPortal::Properties::Axis::X)
                    });
                }
                ...
                if (block == Blocks::Air::toProtocol()
                    ...
                    blocksArray.push_back({
                        {pos.x, pos.y + y, pos.z - 1 + x},
                        Blocks::NetherPortal::toProtocol(Blocks::NetherPortal::Properties::Axis::Z)
                    });
                }
