...
void NetherPortal::setIgnitedBlock() {
    ...
    if (blocksArray[4] == Blocks::Obsidian::toProtocol() && blocksArray[5] != Blocks::Obsidian::toProtocol()) {
        if ((blocksArray[0] == Blocks::Obsidian::toProtocol() && blocksArray[1] != Blocks::Obsidian::toProtocol()) || (blocksArray[2] == Blocks::Obsidian::toProtocol() && blocksArray[3] != Blocks::Obsidian::toProtocol())) {
            _ignitedBlock = PortalIgnition::PORTAL_TOP_LEFT_CORNER;
            return;
        } else if ((blocksArray[1] == Blocks::Obsidian::toProtocol() && blocksArray[0] != Blocks::Obsidian::toProtocol()) || (blocksArray[3] == Blocks::Obsidian::toProtocol() && blocksArray[2] != Blocks::Obsidian::toProtocol())) {
...
void NetherPortal::setBottomLeftCorner() {
    if (_ignitedBlock == PortalIgnition::PORTAL_BOTTOM_LEFT_CORNER) {
        ...
        for (int countY = 1; countY < PortalBoundary::PORTAL_MAX_HEIGHT - PortalBoundary::PORTAL_MIN_HEIGHT - PortalBoundary::PORTAL_NO_CORNERS; countY++) {
            for (int count = 1; count < PortalBoundary::PORTAL_MAX_WIDTH - PortalBoundary::PORTAL_MIN_WIDTH - PortalBoundary::PORTAL_NO_CORNERS; count++) {
                if (_dim->getBlock({_pos.x, _pos.y - countY, _pos.z}) == Blocks::Obsidian::toProtocol() && _dim->getBlock({_pos.x - count, _pos.y, _pos.z}) == Blocks::Obsidian::toProtocol() && count > 1) {
                    _bottomLeftCorner = {_pos.x - count, _pos.y - countY, _pos.z};
                    _direction = PortalDirection::PORTAL_X_AXIS;
                    return;
                } else if (_dim->getBlock({_pos.x, _pos.y - countY, _pos.z}) == Blocks::Obsidian::toProtocol() && _dim->getBlock({_pos.x, _pos.y, _pos.z - count}) == Blocks::Obsidian::toProtocol() && count > 1) {
...
void NetherPortal::setDirection() {
    if (_ignitedBlock == PortalIgnition::PORTAL_NONE || _direction == PortalDirection::PORTAL_WRONG_DIRECTION || _direction == PortalDirection::PORTAL_X_AXIS || _direction == PortalDirection::PORTAL_Z_AXIS)
        return;

    if (_dim->getBlock({_pos.x - 1, _pos.y + 1, _pos.z}) == Blocks::Obsidian::toProtocol() && _dim->getBlock({_pos.x + 1, _pos.y, _pos.z}) == Blocks::Obsidian::toProtocol() && (_dim->getBlock({_pos.x + 2, _pos.y, _pos.z}) == Blocks::Obsidian::toProtocol() || _dim->getBlock({_pos.x + 2, _pos.y + 1, _pos.z}) == Blocks::Obsidian::toProtocol())) {
        _direction = PortalDirection::PORTAL_X_AXIS;
        return;
    } else if (_dim->getBlock({_pos.x, _pos.y + 1, _pos.z - 1}) == Blocks::Obsidian::toProtocol() && _dim->getBlock({_pos.x, _pos.y, _pos.z + 1}) == Blocks::Obsidian::toProtocol() && (_dim->getBlock({_pos.x, _pos.y, _pos.z + 2}) == Blocks::Obsidian::toProtocol() || _dim->getBlock({_pos.x, _pos.y + 1, _pos.z + 2}) == Blocks::Obsidian::toProtocol())) {
        _direction = PortalDirection::PORTAL_Z_AXIS;
        return;
    }
...
void NetherPortal::setSize() {
    ...
    if (_direction == PortalDirection::PORTAL_X_AXIS) {
        for (int countWidth = 1; countWidth < (PortalBoundary::PORTAL_MAX_WIDTH - PortalBoundary::PORTAL_NO_CORNERS); countWidth++) {
            if (_dim->getBlock({_bottomLeftCorner.x + (countWidth - 1), _bottomLeftCorner.y, _bottomLeftCorner.z}) == Blocks::Obsidian::toProtocol() && _dim->getBlock({_bottomLeftCorner.x + countWidth, _bottomLeftCorner.y + 1, _bottomLeftCorner.z}) == Blocks::Obsidian::toProtocol() && countWidth >= (PortalBoundary::PORTAL_MIN_WIDTH - PortalBoundary::PORTAL_NO_CORNERS)) {
        ...
        for (int countHeight = 1; countHeight < (PortalBoundary::PORTAL_MAX_HEIGHT - PortalBoundary::PORTAL_NO_CORNERS); countHeight++) {
            if (_dim->getBlock({_bottomLeftCorner.x - 1, _bottomLeftCorner.y + (countHeight - 1), _bottomLeftCorner.z}) == Blocks::Obsidian::toProtocol() && _dim->getBlock({_bottomLeftCorner.x, _bottomLeftCorner.y + countHeight, _bottomLeftCorner.z}) == Blocks::Obsidian::toProtocol() && countHeight > (PortalBoundary::PORTAL_MIN_HEIGHT - PortalBoundary::PORTAL_NO_CORNERS)) {
                _size.z = countHeight - 1;
            }
        }
    } else if (_direction == PortalDirection::PORTAL_Z_AXIS) {
        for (int countWidth = 1; countWidth < (PortalBoundary::PORTAL_MAX_WIDTH - PortalBoundary::PORTAL_NO_CORNERS); countWidth++) {
            if (_dim->getBlock({_bottomLeftCorner.x, _bottomLeftCorner.y, _bottomLeftCorner.z + (countWidth - 1)}) == Blocks::Obsidian::toProtocol() && _dim->getBlock({_bottomLeftCorner.x, _bottomLeftCorner.y + 1, _bottomLeftCorner.z + countWidth}) == Blocks::Obsidian::toProtocol() && countWidth >= (PortalBoundary::PORTAL_MIN_WIDTH - PortalBoundary::PORTAL_NO_CORNERS)) {
                _size.x = countWidth - 1;
            }
        }
        for (int countHeight = 1; countHeight < (PortalBoundary::PORTAL_MAX_HEIGHT - PortalBoundary::PORTAL_NO_CORNERS); countHeight++) {
...
void NetherPortal::openPortal() {
    ...
    if (_direction == PortalDirection::PORTAL_X_AXIS) {
        ...
        for (auto [position, id] : blocksArray) {
            if (id != Blocks::Air::toProtocol() && id != Blocks::Fire::toProtocol(Blocks::Fire::Properties::Age::ZERO, Blocks::Fire::Properties::East::FALSE, Blocks::Fire::Properties::North::FALSE, Blocks::Fire::Properties::South::FALSE, Blocks::Fire::Properties::Up::FALSE, Blocks::Fire::Properties::West::FALSE)) {
