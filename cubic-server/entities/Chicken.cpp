...
Chicken::Chicken(
    std::shared_ptr<Dimension> dim, u128 uuid, float health, float maxHealth, bool leftHanded, bool aggressive
):
...
void Chicken::layEgg()
{
    ...
    LDEBUG("Chicken will drop egg in {} ticks", time);
    _layEgg.setTickRate(time);
    _dim->makeEntity<Item>(protocol::Slot{ true, ITEM_CONVERTER.fromItemToProtocolId("minecraft:egg"), 1 })
        ->dropItem({ _pos.x, _pos.y + 1, _pos.z }, false);
