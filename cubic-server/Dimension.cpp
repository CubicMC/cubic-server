            ...
            _players.erase(
                std::remove_if(
                    ...
                    [this](const std::shared_ptr<Player> player) {
                        ...
                        return std::find_if(
                                   _idsToRemove.begin(), _idsToRemove.end(),
                                   [playerId](int32_t id) { return id == playerId; }
                               )
                            != _idsToRemove.end();
                    }
                ),
            ...
            _entities.erase(
                std::remove_if(
                    ...
                    [this](const std::shared_ptr<Entity> ent) {
                        ...
                        return std::find_if(
                                   _idsToRemove.begin(), _idsToRemove.end(), [entId](int32_t id) { return id == entId; }
                               )
                            != _idsToRemove.end();
                    }
                ),
...
{
    {
        ...
        _entities.erase(
            std::remove_if(
                _entities.begin(), _entities.end(),
                [entity_id](const std::shared_ptr<Entity> ent) { return entity_id == ent->getId(); }
            ),
            _entities.end()
        );
    }
...
void Dimension::removePlayer(int32_t entity_id)
{
    ...
    _players.erase(
        std::remove_if(
            _players.begin(), _players.end(),
            [entity_id](const std::shared_ptr<Player> ent) { return entity_id == ent->getId(); }
        ),
        _players.end()
    );
    for (auto player : _players)
...
void Dimension::generateChunk(Position2D pos, world_storage::GenerationState goalState)
{
    generateChunk(pos.x, pos.z, goalState);
}
...
void Dimension::loadOrGenerateChunk(int x, int z, const std::shared_ptr<Player> player)
{
    ...
    if (this->_loadingChunks.contains({ x, z })) {
        if (std::find_if(
                this->_loadingChunks[{ x, z }].players.begin(), this->_loadingChunks[{ x, z }].players.end(),
...
void Dimension::removePlayerFromLoadingChunk(const Position2D &pos, const std::shared_ptr<const Player> player)
{
    ...
    if (this->_loadingChunks[pos].players.empty()
        && this->getWorld()->getGenerationPool().cancelJob(this->_loadingChunks[pos].id)) {
        // this->_loadingChunks[pos].task->cancel();
        // This could be replaced using either an iterator, or something else (maybe an if condition inside the job? or
        // simply integrated inside the overlay.)
...
world_storage::ChunkColumn &Dimension::getChunk(const Position2D &pos)
{
    return this->_level.getChunkColumn(pos);
}

const world_storage::ChunkColumn &Dimension::getChunk(int x, int z) const
{
    return this->_level.getChunkColumn(x, z);
}
const world_storage::ChunkColumn &Dimension::getChunk(const Position2D &pos) const
{
    return this->_level.getChunkColumn(pos);
}
    ...
    auto current_id = current.getId();
    {
        ...
        for (auto player : _players) {
            ...
            if (player->getId() != current_id) {
                player->sendSpawnPlayer({ current_id, current.getUuid(), current.getPosition().x,
                                          current.getPosition().y, current.getPosition().z, current.getRotation().x,
                                          current.getRotation().z });
                LDEBUG("send spawn player to {}", player->getUsername());
                current.sendSpawnPlayer({ player->getId(), player->getUuid(), player->getPosition().x,
                                          player->getPosition().y, player->getPosition().z, player->getRotation().x,
...
void Dimension::updateBlock(Position position, int32_t id)
{
    ...
    if (id == 0)
        ...
        if (convertBlockNameToBlockEntityType(GLOBAL_PALETTE.fromProtocolIdToBlock(id).name)
            != TileEntityType::UnknownType) {
...
Tps Dimension::getTps() const
{
    ...
    const auto tpsCalculation = [buffer_end](const int tick_number) {
        return 1.0f
            / ((std::accumulate(buffer_end - tick_number, buffer_end, 0.0f) / (float) (tick_number))
               / MICROSECS_IN_ONE_SEC);
    };
