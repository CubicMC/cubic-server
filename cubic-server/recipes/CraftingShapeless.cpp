    ...
    Recipe(identifier, recipe)
{
    ...
    if (!recipe.contains("ingredients") || !recipe.contains("result") || !recipe["ingredients"].is_array()
    ...
    for (size_t i = 0; i < recipe["ingredients"].size(); i++) {
        if (recipe["ingredients"].at(i).is_object() && recipe["ingredients"].at(i).contains("item")
            && recipe["ingredients"].at(i)["item"].is_string())
            this->_ingredients.at(i).insert(
                ITEM_CONVERTER.fromItemToProtocolId(recipe["ingredients"].at(i)["item"].get<std::string>())
            );
            ...
            for (const auto &ingredient : recipe["ingredients"].at(i)) {
                if (ingredient.is_object() && ingredient.contains("item") && ingredient["item"].is_string())
                    this->_ingredients[i].insert(
                        ITEM_CONVERTER.fromItemToProtocolId(ingredient["item"].get<std::string>())
                    );
            }
        } else
            return;
...
const std::vector<std::unordered_set<ItemId>> &CraftingShapeless::getIngredients() const
{
    return (this->_ingredients);
}

ItemId CraftingShapeless::getCraftedItem() const noexcept
{
    return (this->_result);
}

uint64_t CraftingShapeless::getCraftedItemCount() const noexcept
{
    return (this->_count);
}
} // namespace Recipe
