    ...
    Recipe(recipe)
{
    ...
    if (!recipe.contains("ingredient") || !recipe.contains("result") || !recipe.contains("experience") || !recipe.contains("cookingtime") || !recipe["ingredient"].is_object() ||
        !recipe["result"].is_string() || !recipe["experience"].is_number() || !recipe["cookingtime"].is_number_unsigned() || !recipe["ingredient"].contains("item") ||
...
{
    LINFO(
        "\"", Server::getInstance()->getItemConverter().fromProtocolIdToItem(this->_ingredient), "\" -> \"",
        Server::getInstance()->getItemConverter().fromProtocolIdToItem(this->_result), "\" (cooking for ", this->_cookingTime, " ticks and get ", this->_experience, " xp)"
    );
}
