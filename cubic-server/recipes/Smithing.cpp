    ...
    Recipe(recipe)
{
    ...
    if (!recipe.contains("base") || !recipe.contains("addition") || !recipe.contains("result") || !recipe["base"].is_object() || !recipe["addition"].is_object() ||
        !recipe["result"].is_object() || !recipe["base"].contains("item") || !recipe["base"]["item"].is_string() || !recipe["addition"].contains("item") ||
        !recipe["addition"]["item"].is_string() || !recipe["result"].contains("item") || !recipe["result"]["item"].is_string())
        return;
...
{
    LINFO(
        "\"", Server::getInstance()->getItemConverter().fromProtocolIdToItem(this->_base), "\" + \"",
        Server::getInstance()->getItemConverter().fromProtocolIdToItem(this->_addition), "\" = \"", Server::getInstance()->getItemConverter().fromProtocolIdToItem(this->_result),
        "\""
    );
}
