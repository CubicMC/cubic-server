...
namespace Recipe {
    Smoking::Smoking(const std::string &identifier, const nlohmann::json &recipe):
    ...
    {
        LINFO("send minecraft:smoking: ", this->getIdentifier());
        ...
        protocol::serialize(payload,
            "minecraft:smoking", protocol::addIdentifier,
    ...
    std::unique_ptr<Recipe> Smoking::create(const std::string &identifier, const nlohmann::json &recipe)
    {
        return (std::make_unique<Smoking>(Smoking(identifier, recipe)));
    }
};