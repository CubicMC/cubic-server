...
namespace Recipe {
    ...
    void Smelting::insertToPayload(std::vector<uint8_t> &payload) const
    {
        LINFO("send minecraft:smelting: ", this->getIdentifier());
        ...
        protocol::serialize(payload,
            "minecraft:smelting", protocol::addIdentifier,
    ...
    std::unique_ptr<Recipe> Smelting::create(const std::string &identifier, const nlohmann::json &recipe)
    {
        return (std::make_unique<Smelting>(Smelting(identifier, recipe)));
    }
};