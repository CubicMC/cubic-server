    ...
    Recipe(recipe)
{
    ...
    if (!recipe.contains("ingredient") || !recipe.contains("result") || !recipe.contains("count") || !recipe["ingredient"].is_object() || !recipe["result"].is_string() ||
        !recipe["count"].is_number_unsigned() || !recipe["ingredient"].contains("item") || !recipe["ingredient"]["item"].is_string())
        return;
...
void StoneCutting::dump(void) const
{
    LINFO(
        ...
        Server::getInstance()->getItemConverter().fromProtocolIdToItem(this->_result), "\" (x", this->_count, ")"
    );
}
