...
#ifdef __linux__
   void ThreadWorker::_nameThread() { prctl(PR_SET_NAME, reinterpret_cast<unsigned long>((_name + "|" + std::to_string(_threadWorkerId)).c_str())); } // only way to set thread name under all linux (no POSIX standard).
#else
...
ThreadWorker::ThreadWorker(size_t index, ThreadPoolUtility &toolBox, const std::string_view name) : _toolBox(toolBox), _name(name), _threadWorkerId(index)
{
