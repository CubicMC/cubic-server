...
namespace thread_pool {
   ...
   public:
      ...
      // we fetch target size, as it is the size the threadpool will converge to. the real size is only temporary as the threadPool cannot modify the thread number outside of its construction.
      int getWorkerNb() const { std::lock_guard<std::mutex> _(_toolBox.sizeProtection); return _toolBox.targetSize; }

      // any modifications will only take place AFTER a worker has ended its shift.
      void changeWorkerNb(uint16_t newSize) { std::lock_guard<std::mutex> _(_toolBox.sizeProtection); _toolBox.targetSize = newSize; }

      // no verification for overflow. if you ever try to add more than UNSIGNED_16_BITS_MAX, you might want to rethink your life.
      void addWorker() { std::lock_guard<std::mutex> _(_toolBox.sizeProtection); ++_toolBox.targetSize; }

      // allow trying to remove worker when there's none. that's not important
      void removeWorker() { std::lock_guard<std::mutex> _(_toolBox.sizeProtection); if (_toolBox.targetSize > 0) --_toolBox.targetSize; }
      ...
      void addJob(int priority, sameFunction<helperSameFunction<void(void)>> auto ...job) { addJob([priority]{ return priority; }, job...); }
