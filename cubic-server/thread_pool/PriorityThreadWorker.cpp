...
#ifdef __linux__
   void PriorityThreadWorker::_nameThread() { prctl(PR_SET_NAME, reinterpret_cast<unsigned long>((_name + "|" + std::to_string(_threadWorkerId)).c_str())); } // only way to set thread name under all linux (no POSIX standard).
#else
...
void PriorityThreadWorker::doJob()
{
   ...
   do {
      ...
      try { job(); } catch (const std::exception &) { /* TODO: add either a logger or an exception propagation here */ }
      jobList.pop();
   } while (!jobList.empty());
...
PriorityThreadWorker::PriorityThreadWorker(size_t index, PriorityThreadPoolUtility &toolBox, const std::string_view name) : _toolBox(toolBox), _name(name), _threadWorkerId(index)
{
