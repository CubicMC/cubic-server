...
namespace thread_pool {
   ...
   public:
      ...
      void setSelf(std::list<PriorityThreadWorker>::iterator self) { if (_self != _toolBox.workers.end()) throw thread_pool::JobException("Trying to set an already setted iterator on worker"); _self = self; _isInit.count_down(); }
   };
}
