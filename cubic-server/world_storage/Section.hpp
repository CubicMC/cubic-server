...
constexpr uint64_t calculateSectionBlockIdx(const Position &pos)
{
    return pos.x + (pos.z * SECTION_WIDTH) + (pos.y * SECTION_2D_SIZE);
}
constexpr uint64_t calculateSectionBiomeIdx(const Position &pos)
{
    return pos.x + (pos.z * BIOME_SECTION_WIDTH) + (pos.y * BIOME_SECTION_2D_SIZE);
}

constexpr const Position calculateAbsolutePosition(
    const uint32_t blockIndex, const Position2D &chunkPosition, const size_t sectionIndex
)
{
    ...
    return { chunkPosition.x * SECTION_WIDTH + sectionX, y + CHUNK_HEIGHT_MIN - 16,
             chunkPosition.z * SECTION_WIDTH + sectionZ };
}
...
public:
    ...
    [[nodiscard]]
    int32_t getBlock(uint64_t idx) const;
    [[nodiscard]]
    int32_t getBiome(uint64_t idx) const;
    ...
    [[nodiscard]]
    uint8_t getBlockLight(uint64_t idx) const;
    [[nodiscard]]
    uint8_t getSkyLight(uint64_t idx) const;

    [[nodiscard]]
    inline constexpr bool hasBlocks() const
    {
        return _blockPalette.getBits() != 0;
    }
    [[nodiscard]]
    inline constexpr bool hasBiomes() const
    {
        return _biomePalette.getBits() != 0;
    }

    [[nodiscard]]
    inline BlockStorage &getBlocks()
    {
        return _blocks;
    }
    [[nodiscard]]
    inline BiomeStorage &getBiomes()
    {
        return _biomes;
    }

    [[nodiscard]]
    inline const BlockStorage &getBlocks() const
    {
        return _blocks;
    }
    [[nodiscard]]
    inline const BiomeStorage &getBiomes() const
    {
        return _biomes;
    }

    [[nodiscard]]
    inline BlockPalette &getBlockPalette()
    {
        return _blockPalette;
    }
    [[nodiscard]]
    inline BiomePalette &getBiomePalette()
    {
        return _biomePalette;
    }

    [[nodiscard]]
    inline const BlockPalette &getBlockPalette() const
    {
        return _blockPalette;
    }
    [[nodiscard]]
    inline const BiomePalette &getBiomePalette() const
    {
        return _biomePalette;
    }
    ...
    {
        return _skyLightCount > 0;
    }
    [[nodiscard]]
    inline bool hasBlockLight() const
    {
        return _blockLightCount > 0;
    }

    [[nodiscard]]
    inline LightStorage &getBlockLights()
    {
        return _blockLight;
    }
    [[nodiscard]]
    inline LightStorage &getSkyLights()
    {
        return _skyLight;
    }

    [[nodiscard]]
    inline const LightStorage &getBlockLights() const
    {
        return _blockLight;
    }
    [[nodiscard]]
    inline const LightStorage &getSkyLights() const
    {
        return _skyLight;
    }
