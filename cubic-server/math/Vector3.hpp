...
public:
    ...
    {
        return std::sqrt(
            std::pow((other.x - this->x), 2) + std::pow((other.y - this->y), 2) + std::pow((other.z - this->z), 2)
        );
    }
    ...
    constexpr T dotProduct(const Vector3 &other) noexcept
    {
    ...
    {
        return Vector3<T>(other + this->x, other + this->y, other + this->z);
    }
