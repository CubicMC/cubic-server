...
int compressVector(const std::vector<uint8_t> &source, std::vector<uint8_t> &destination)
{
    ...
    int return_value = compress2(
        (Bytef *) destination_data, &destination_length, source_data, source_length,
        CONFIG["compression-level"].as<int>()
    );
...
)
{
    ...
    uLongf maxSize = size;
    int return_value = uncompress(
        dest, &maxSize, (const Bytef *) compressedBytes.data(), (uLong) compressedBytes.size()
    );
